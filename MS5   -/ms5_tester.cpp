...
void testProduct() {
  ...
  if (ok) {
    ...
    product.store(productFile); // pname proderror null  undefined - pdes sku
    productFile.close();
	//tprd.errorcheck();   // added errorchecker
    ...
    tprd.load(productFile); // set here? sku name pdes correct tstate// instock undefined , needed undefined price undefined proderror
    cout << " Program: 1234   |box                 | 139.50|   1|kg        |   5|" << endl;
