...
namespace sict {
	...
	Contact::Contact(const Contact& holder) {

		strncpy(name, holder.name, 19);
		size = holder.size;
		if (holder.pnum != nullptr) {

			pnum = new long long[size];
			for (int i = 0; i < size; i++) pnum[i] = holder.pnum[i];
		}
		else pnum = nullptr;
	}
	
	Contact& Contact::operator = (const Contact& comp) {
		
		if (!isEmpty() && this != &comp) {
			strncpy(name, comp.name, strlen(comp.name));
			name[strlen(comp.name)] = char(0);

			
			pnum = new long long[size = comp.size];
	...
	Contact &Contact:: operator+= (long long num) {
			if (validate(num)) {
				...
				long long* numholder = nullptr;
					numholder = new long long[size + 1];
					for (int i = 0; i < size + 1; i++)
				
						if (pnum != nullptr) numholder[i] = pnum[i];
					numholder[size] = num;
					Contact holder(name, numholder, size + 1);
